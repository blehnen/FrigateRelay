                ...
                withCredentials([string(credentialsId: 'reportgenerator-license', variable: 'REPORTGENERATOR_LICENSE')]) {
                    sh '''
                        ...
                        .dotnet-tools/reportgenerator \
                            ...
                            -assemblyfilters:"-FrigateRelay.TestHelpers" \
                            -license:"$REPORTGENERATOR_LICENSE"
                    '''
                }
                // -assemblyfilters: TestHelpers is shared test-utility code (CapturingLogger
                // and friends), not part of the runtime service surface, so it does not belong
                // in the production coverage metric. The filter applies to BOTH the merged HTML
                // report and the merged Cobertura.xml that Codecov ingests — single point of
                // exclusion.
