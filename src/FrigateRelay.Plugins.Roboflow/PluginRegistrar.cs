...
/// entries with <c>Type == "Roboflow"</c>, and registers one keyed
...
/// (CPAI, DOODS2) coexist by filtering on their own <c>Type</c> discriminator value.
/// </para>
/// <para>
...
/// Single <see cref="RoboflowOptions.Timeout"/>; fail-{closed,open} per
/// <see cref="RoboflowOptions.OnError"/> (CONTEXT-7 D4 / CONTEXT-14 architect lock-in).
/// </para>
/// </remarks>
public sealed class PluginRegistrar : IPluginRegistrar
{
    ...
    public void Register(PluginRegistrationContext context)
    {
        ...
        foreach (var instance in validatorsSection.GetChildren())
        {
            ...
            if (!string.Equals(type, "Roboflow", StringComparison.Ordinal))
                continue;
            ...
            context.Services
                .AddOptions<RoboflowOptions>(instanceKey)
                .Bind(instance)
            ...
            var clientName = $"Roboflow:{instanceKey}";
            context.Services
                .AddHttpClient(clientName, (sp, client) =>
                {
                    var opts = sp.GetRequiredService<IOptionsMonitor<RoboflowOptions>>().Get(instanceKey);
...
#pragma warning restore CA5359
            ...
            {
                var name = (string)key!;
                var opts = sp.GetRequiredService<IOptionsMonitor<RoboflowOptions>>().Get(name);
                var http = sp.GetRequiredService<IHttpClientFactory>().CreateClient($"Roboflow:{name}");
                var logger = sp.GetRequiredService<ILogger<RoboflowValidator>>();
                return new RoboflowValidator(name, opts, http, logger);
            });
        }
    }
}
