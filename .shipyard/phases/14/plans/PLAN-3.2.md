...
- Host-shutdown propagation: `OperationCanceledException when ct.IsCancellationRequested` from any validator MUST bubble out of `Task.WhenAll` and unwind the consumer loop (existing behavior preserved). `Task.WhenAll` rethrows the first exception by default; that's the desired semantic here.
