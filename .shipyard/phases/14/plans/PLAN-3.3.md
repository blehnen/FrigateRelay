...
- Both validator requests' timestamps are within a small window (e.g. 100ms of each other) — empirical evidence the validators ran concurrently, not sequentially. Use the WireMock log entries' `RequestMessage.DateTime` field for timestamps; cross-reference the two timestamps' delta to assert it's well below `cpai_timeout + roboflow_timeout` (sequential lower bound).
