...
- **#19 — Name-allowlist enforcement (`StartupValidation.cs`, new `ValidateNames` pass).** Enforce `^[A-Za-z0-9_. -]+$` (D1 permissive-printable, per CONTEXT-15) for subscription, profile, plugin, and validator names; preserves spaced names like `"DriveWay Person"` while rejecting CRLF, slashes, colons, and at-signs. Bundles with #13 because both target the structured-logging boundary; the name allowlist is the *structural* fix (illegal characters never reach a log/span tag) and the sanitization helper is the *defensive* fix (operator-controlled strings flowing through future code paths cannot inject newlines). New pass appends to the shared `errors` list per the D7 collect-all pattern; called from `ValidateAll`.
...
## Phase 16 — v1.3.0 Minor Release  *[BUILD COMPLETE 2026-05-08 — awaiting PR + tag]*

**Status.** Build complete on `main` 2026-05-08 (11 commits: 9 plan commits + 1 doc-additions follow-up + 1 orchestrator inline test-fix). 313 tests pass across 11 projects (Host 151→154, CPAI 8→13, Roboflow 16, DOODS2 14, others unchanged). Quality gates: Verifier PASS, Audit PASS_WITH_NOTES (3 advisory deferred), Simplifier MINOR_BLOAT (1 High — `StaticOptionsMonitor<T>` extraction — explicitly deferred to Phase 17 first task), Documenter GAPS_NON_BLOCKING (5 recommendations all applied in commit `0d46303`). Awaiting `/shipyard:ship` for PR + manual `v1.3.0` tag-cut per CONTEXT-16 D9. Original scope description retained below for posterity.

Phase 16 is the implementation phase for the v1.3.0 scope captured in `.shipyard/phases/15-and-16-issues-cleanup-design.md` (Phase 16 section). Single batch covers three ISSUES.md entries: #18 (operator-visible `MetricsTags:KnownCameras` config key — the SemVer-minor justification), #22 (test-fragility cleanup in `tests/FrigateRelay.Host.Tests/Observability/`), and #30 (atomic 3-file `PluginRegistrar` registration-shape unification across CPAI + Roboflow + DOODS2). Lower risk than Phase 15 because #22 + #30 are zero-shipping-impact and #18 is gated behind a default-empty opt-in config key.
