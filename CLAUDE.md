...
- **`Subscriptions:N:Actions` requires the object form** `[{ "Plugin": "BlueIris" }, { "Plugin": "Pushover", "SnapshotProvider": "Frigate" }]`. The legacy string-array form `["BlueIris"]` is parsed correctly by `ActionEntryJsonConverter` for direct `JsonSerializer.Deserialize`, but `IConfiguration.Bind` does NOT invoke `[JsonConverter]` attributes — so the string-array shape silently produces an empty `Actions` list at runtime, dropping action firing without any error. Tracked as ID-12 in `.shipyard/ISSUES.md`. Operators upgrading from Phase 4 must migrate `appsettings.json` to the object form. The fix (a custom `TypeConverter` for `ActionEntry`) is deferred — until then, validate config samples and integration test fixtures with the object shape only.
