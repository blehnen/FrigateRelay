...
- **`CapturingLogger<T>.WaitForEntriesAsync(count, timeout)` replaces fixed-time `Task.Delay` polling in async observability tests.** Call `await logger.WaitForEntriesAsync(1, TimeSpan.FromSeconds(2))` to wait until at least `count` log entries have been captured or the timeout elapses. Polls `Entries.Count` at 25ms intervals; throws `TimeoutException` on deadline; `OperationCanceledException` propagates naturally from the inner `Task.Delay`. For code paths that emit no log message (e.g. the dispatcher success path), use a `MeterListener`-based `TaskCompletionSource` fallback that resolves on the first terminal counter measurement (`actions.succeeded`, `actions.failed`, `actions.exhausted`, `validators.rejected`). Canonical pattern: `tests/FrigateRelay.Host.Tests/Observability/CounterIncrementTests.cs:RunDispatcherAsync`. The greppable invariant `git grep -nE 'Task\.Delay\([0-9]' tests/FrigateRelay.Host.Tests/Observability/` must return empty (the 2 `Task.Delay(Timeout.Infinite, ct)` cancellation-await sites in fake `IEventSource` stubs are structurally correct and intentionally retained). Established Phase 16 (#22).
- **New required constructor parameters go ahead of existing optional/nullable trailing parameters — and use named arguments at all downstream call sites.** Inserting a required parameter before an existing optional parameter breaks any positional caller (`CS1503` if the types differ; silent argument-shift if they match). When adding a required parameter to a constructor that already has an `ISnapshotResolver?` or similar nullable trailing parameter: (1) place the new required param immediately before the optional one; (2) update every downstream call site to use named arguments (`snapshotResolver: null`) or insert the new arg in the correct slot. Precedent: `ChannelActionDispatcher` gained `MetricsTagWriter` at position 4 in Phase 16, pushing `ISnapshotResolver?` to position 5 — one positional test caller had to be migrated to named-argument form.
- **`StaticOptionsMonitor<T>` is a Phase 17 pending extraction.** A 4-line `IOptionsMonitor<T>` stub (`CurrentValue`, `Get`, `OnChange → null`) is currently duplicated across 7 test files as either `StaticMonitor<T>` or `StaticOptionsMonitor<T>` (11 nested-class definitions in total, all identical-shape after Phase 16). Do NOT define a new copy in a new test file; reuse an existing per-file copy within the same test class file until the canonical version lands at `tests/FrigateRelay.TestHelpers/StaticOptionsMonitor.cs`. Same precedent as `CapturingLogger<T>` (extracted Phase 6 after 4 duplicates accumulated). Tracked in `.shipyard/phases/16/results/SIMPLIFICATION-16.md` High finding.
